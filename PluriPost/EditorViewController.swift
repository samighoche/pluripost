...
class EditorViewController: UIViewController,UIScrollViewDelegate,UIAlertViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate
{
    ...
    func handleSpaceTap(recognizer:UITapGestureRecognizer)
    {
        ...
        if (mode == "edit" && currentSpace == mySpace)
        {
        ...
        containerView.frame = currentSpace.frame
    ...
    @IBAction func backToCardViewAction(sender: AnyObject) {
        ...
        containerView.frame = CGRectMake(0.0, 0.0, scrollView.frame.width, scrollView.frame.height)
        
        currentSpace.frame = currentSpaceFrame
        currentSpace.center = CGPointMake(containerView.frame.width/2, currentSpace.center.y)
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        dispatch_async(queue) {
            ...
            dispatch_async(dispatch_get_main_queue(),{
                if (D_spaceIndex >= 0)
                {
                    self.zoomtoSpace(spaces[D_spaceIndex])
    ...
    override func viewDidLoad() {
        ...
        if (D_firstTimeInEditor)
        {
            for index in 1...(D_selectedFriends.count + 1)
            {
                var space : UIView = UIView(frame: CGRectMake(containerView.frame.width/2, CGFloat(20*index + 138*(index-1)), containerView.frame.width-30.0, 138.0))
                space.center = CGPointMake(containerView.frame.width/2, space.center.y)
                ...
                {
                    var profileSpace:UIImageView = UIImageView()
                    profileSpace.image = UIImage(named: (D_selectedFriends[index - 2].userImageName as String))
                    profileSpace.frame = CGRectMake(4.0, 4.0,25.0, 25.0)
                    profileSpace.layer.cornerRadius = 12.5
                    profileSpace.clipsToBounds = true
                    space.addSubview(profileSpace)
                    
                    var profilePicView:UIImageView = UIImageView(frame: CGRectMake(space.frame.origin.x/2, space.frame.origin.y/2, 50.0, 50.0))
                    /*profilePicView.clipsToBounds = true
                    profilePicView.layer.cornerRadius = 25.0
                    profilePicView.image = UIImage(named: (D_selectedFriends[index-2] as user).userImageName as String)
        ...
        if (mode == "preview")
        {
            ...
            cardImagePreviewView.image = D_cardImage
    ...
    func createImageView() -> UIImageView
    {
        ...
        view.center = CGPointMake(containerView.frame.width/2, containerView.frame.height/2 - 25.0)
    ...
    {
        var view : UITextView = UITextView(frame: CGRect(origin: CGPoint(x:containerView.frame.width/2,y:containerView.frame.height/2), size: CGSize(width: 200.0, height: 150.0)))
        view.font = UIFont(name: "BradleyHandITCTT-Bold", size: 18)
        view.center = CGPointMake(containerView.frame.width/2, containerView.frame.height/2 - 75.0)
    ...
    func createVideoView() -> VideoView
    {
        ...
        view.frame = CGRect(origin: CGPoint(x:containerView.frame.width/2,y:containerView.frame.height/2), size: CGSize(width: 150, height: 150))
        view.center = CGPointMake(containerView.frame.width/2, containerView.frame.height/2)
        view.exclusiveTouch = true
        view.userInteractionEnabled = true
        currentSpace.addSubview(view)
    ...
    func handleLongPress(recognizer:UILongPressGestureRecognizer) {
        ...
        if (!currentlyDeletingOrResizing && mode == "edit")
        {
    ...
    func centerScrollViewContents() {
        ...
        //var contentsFrame = spaces[0].frame
    ...
    @IBAction func sign(sender: AnyObject) {
        ...
        alert.addAction(UIAlertAction(title: "Proceed", style: UIAlertActionStyle.Default){
            UIAlertAction in D_didEndEditing = true
